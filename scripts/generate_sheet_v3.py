...
Usage (from repository root):
  python scripts/generate_sheet_v3.py
  python scripts/generate_sheet_v3.py --pdf   (needs playwright)
"""
...
from omr_paths import BUBBLE_COORDS_DIR
...
if __name__ == "__main__":
    ...
    sheets_dir = _REPO / "sheets"
    sheets_dir.mkdir(parents=True, exist_ok=True)
    coords_dir = BUBBLE_COORDS_DIR
    ...
    for n in sizes:
        for lang in ("en", "ar"):
            ...
            html_path = sheets_dir / f"{stem}.html"
            html_path.write_text(html, encoding="utf-8")
            print(f"[{lang.upper()} {n:>3}Q] -> {html_path}")
            if args.pdf:
                pdf_path = sheets_dir / f"{stem}.pdf"
                to_pdf(str(html_path), str(pdf_path))

                rtl = (lang == "ar")
                coords = extract_bubble_coords(
                    str(html_path), n, 5, lp["n_cols"], rtl)
